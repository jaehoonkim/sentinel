...
func (ctl ControlVanilla) PollingService(ctx echo.Context) error {
	...
	clusterInfo := func(fn func(clusterinfov2.ClusterInformation) (int, time.Time, []servicev4.HttpRsp_AgentServicePolling, error)) (cnt int, v []servicev4.HttpRsp_AgentServicePolling, err error) {
	...
	makePollingServicev3 := func(cluster_info clusterinfov2.ClusterInformation) (polling_cnt int, polling_offset time.Time, rsp_ []servicev4.HttpRsp_AgentServicePolling, err error) {
		...
		// make response body
		rsp := make([]servicev3.HttpRsp_AgentServicePolling, len(servs))
		...
		rsp_ = make([]servicev4.HttpRsp_AgentServicePolling, len(servs))
		for i := range rsp {
			rsp_[i].Version = "v3"
			rsp_[i].V3 = rsp[i]
		}
	...
	makePollingServicev4 := func(cluster_info clusterinfov2.ClusterInformation) (polling_cnt int, polling_offset time.Time, rsp []servicev4.HttpRsp_AgentServicePolling, err error) {
		...
		// make response body
		rsp = make([]servicev4.HttpRsp_AgentServicePolling, len(servs))
		for i := range servs {
			rsp[i].Version = "v4"
			rsp[i].V4 = servs[i]
		}
	...
	rspv3 := make([]servicev3.HttpRsp_AgentServicePolling, len(rsp))
	for i, s := range rsp {
		rspv3[i] = s.V3
	}
...
func (ctl ControlVanilla) UpdateService(ctx echo.Context) (err error) {
	...
	body := servicev4.HttpReq_AgentServiceUpdate{}
	...
	updateServiceStatus_v3 := func(body servicev3.HttpReq_AgentServiceUpdate) error {
		...
		stepStatus := func(body servicev3.HttpReq_AgentServiceUpdate, serv servicev3.Service, service_step servicev3.ServiceStep) servicev3.StepStatus {
			// 스탭 포지션이 카운트와 같은 경우만
		...
		serviceResult := func(body servicev3.HttpReq_AgentServiceUpdate) cryptov2.CryptoString {
			// 상태가 성공인 경우만
		...
		stepMessage := func(body servicev3.HttpReq_AgentServiceUpdate) vanilla.NullString {
			// 상태가 실패인 경우만
		...
		resultType := func(body servicev3.HttpReq_AgentServiceUpdate, serv servicev3.Service, service_step servicev3.ServiceStep) (resultType servicev3.ResultSaveType) {
			// 마지막 스탭의 결과만 저장 한다
			if serv.StepCount != stepPosition(service_step) {
				return
			}
	...
	updateServiceStatus_v4 := func(body servicev4.HttpReq_AgentServiceUpdate_multistep) error {
		if err := body.Status.Valid(); err != nil {
			err = errors.Wrapf(err, "valid param%s",
				logs.KVL(
					ParamLog(fmt.Sprintf("%s.Status", TypeName(body)), body.Status)...,
